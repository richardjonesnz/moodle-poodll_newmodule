...
/**
 * Action for adding/editing a NEWMODULE attempt.
 *
...
$PAGE->set_url('/mod/NEWMODULE/manageattempts.php', array('attemptid'=>$attemptid, 'n'=>$n,'action'=>$action));

//make sure we are logged in and can see this form
require_login($course, false, $cm);
$context = context_module::instance($cm->id);
require_capability('mod/NEWMODULE:manageattempts', $context);
...
if ($action=='delete' && $attemptid > 0) {
    $attempt = $DB->get_record(constants::M_USERTABLE, array('id'=>$attemptid,'NEWMODULEid' => $cm->instance), '*', MUST_EXIST);
	if(!$attempt){
...
//we always head back to the NEWMODULE attempts page
switch($source){
	case 'attempts':
		$redirecturl = new moodle_url('/mod/NEWMODULE/reports.php', array('report'=>'attempts','id'=>$cm->id,'n'=>$n));
		break;
	case 'grading':
		$redirecturl = new moodle_url('/mod/NEWMODULE/grading.php', array('id'=>$cm->id,'action'=>'grading'));
		break;
	case 'gradingbyuser':
		$redirecturl = new moodle_url('/mod/NEWMODULE/grading.php', array('action'=>'gradingbyuser','userid'=>$attempt->userid,'n'=>$n));
		break;
}
...
/////// Delete attempt NOW////////
	...
	case 'deleteall':
		require_sesskey();
		if (!$DB->delete_records(constants::M_USERTABLE, array('NEWMODULEid'=>$moduleinstance->id))){
			print_error("Could not delete attempts (all)");
		}
        //delete AI grades for this activity too
        $DB->delete_records(constants::M_AITABLE, array('NEWMODULEid'=>$moduleinstance->id));
